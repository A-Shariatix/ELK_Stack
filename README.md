...
- #### environment:
  **ELASTICSEARCH_HOSTS** -> Specifies the Elasticsearch's host address.<br>
  **ELASTICSEARCH_USERNAME** -> Specifies an Elasticsearch username, which is "kibana_system" in this case. "kibana_system" is a pre-built user designed with required priveleges for Kibana.<br>
...
- #### volumes:
  In this project, Kibana uses a single bind mount with the purpose of mounting the certs and the CA from certs directory on the host into the /usr/share/kibana/config/kibana path.<br>
